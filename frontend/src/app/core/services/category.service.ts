...
export class CategoryService {
  ...
  create(dto: { name: string }): Observable<ICategory> {
    return this.http.post<ICategory>(this.apiUrl, dto).pipe(
      catchError(this.handleError)
    );
  }

  update(id: number, dto: { name: string }): Observable<ICategory> {
    return this.http.put<ICategory>(`${this.apiUrl}/${id}`, dto).pipe(
      catchError(this.handleError)
    );
  }

  delete(id: number): Observable<ICategory> {
    return this.http.delete<ICategory>(`${this.apiUrl}/${id}`).pipe(
      catchError(this.handleError)
    );
  }
}
